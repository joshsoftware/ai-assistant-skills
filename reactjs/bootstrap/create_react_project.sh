...
#   2. clones the `.claude` toolkit (skills/agents/commands/hooks) and the
#      canonical `CLAUDE.md` from this git repo into the project.
#
...
CLAUDE_MD_SRC="reactjs/bfsi-boilerplate/CLAUDE.md"
...
if [ ! -f "$TEMP_DIR/$CLAUDE_MD_SRC" ]; then
    echo "Error: '$CLAUDE_MD_SRC' not found in the repo."
    exit 1
fi
...
if [ "$AI_TOOL" = "claude" ]; then
    ...
    cp "$TEMP_DIR/$CLAUDE_MD_SRC" "$PROJECT_NAME/CLAUDE.md"
else
